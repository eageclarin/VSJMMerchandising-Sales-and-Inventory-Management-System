...
class PDF extends FPDF{
    function Header(){
        ...
        $this->Ln(10);
        $this->Cell(190,10,'Quarterly Sales Report',50,0,'C');
...
$result = mysqli_query($conn, $sql);
    ...
    $pdf->SetFont('Arial','B',8);
        ...
        $y = $pdf->GetY();
        $date = date("Y-m-d");
        $sql = "SELECT QUARTER(sales_Date) AS quarter, YEAR(sales_Date) AS year, sales_Date, COUNT(DISTINCT sales.sales_ID) AS totalOrders, SUM(salesItems_Quantity) AS totalItems, SUM(salesItems_TotalPrice) AS totalSales FROM sales INNER JOIN sales_items ON (sales.sales_ID = sales_items.sales_ID) GROUP BY QUARTER(sales_Date), YEAR(sales_Date)" ;
        $result = mysqli_query($conn,$sql);
